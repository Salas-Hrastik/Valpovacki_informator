-- ===========================================================================
-- AI asistent ljudskih potencijala — shema baze (Supabase / PostgreSQL)
--
...
-- ---------------------------------------------------------------------------
-- Tablica: dokumenti — jedan redak po ODJELJKU KNJIGE (poglavlje/potpoglavlje)
--   url:   stabilna interna referenca odjeljka, npr. "knjiga://mlp/knjiga/012-analiza-posla"
--   title: naslov odjeljka s rasponom stranica, npr. "Analiza posla (str. 45–61)"
--   pages: raspon stranica u knjizi (dijagnostika/citati)
-- ---------------------------------------------------------------------------
create table if not exists dokumenti (
  id            uuid primary key default gen_random_uuid(),
  url           text not null unique,
  title         text not null default '',
  source        text not null default '',          -- naziv izvorne datoteke knjige
  lang          text not null default 'hr',
  pages         text not null default '',          -- npr. "45–61"
  content_text  text not null default '',          -- očišćeni tekst odjeljka
  content_hash  text not null,                     -- SHA-256 teksta (detekcija promjena)
  fetched_at    timestamptz not null default now(),-- kada je odjeljak zadnji put ingestiran
...
-- ---------------------------------------------------------------------------
-- Tablica: dijelovi — tekstualni isječci (chunkovi) odjeljka
...
-- na 5 znakova) tolerira sklonidbu: "selek:*" hvata "selekcija", "selekcijski".
...
-- Transakcijski upsert odjeljka s isječcima i ugradnjama ------------------------
...
declare
  v_doc_id uuid;
begin
  insert into dokumenti (url, title, source, lang, pages,
