...
interface Source {
  title: string;
  url: string; // interna referenca odjeljka (knjiga://…) ili web poveznica
  score: number;
  fetched_at: string;
}
...
// Brzi prijedlozi pitanja — prikazuju se na početku da korisnik odmah vidi
...
const PRIJEDLOZI = [
  'Što je menadžment ljudskih potencijala i koje su mu funkcije?',
  'Koje metode selekcije kandidata postoje?',
  'Ispitaj me iz motivacije zaposlenika!',
];
...
export default function Chat() {
  ...
  // Otvara poveznicu iz odgovora u skočnom pregledniku umjesto u novoj kartici.
  // Vraća handler za onClick koji spriječi zadanu navigaciju.
  ...
  return (
    <div className="chat" aria-busy={busy}>
      ...
      <div
        ...
        aria-label="Razgovor s asistenticom"
      >
        {messages.map((m, i) => (
          <div key={i} className={`msg msg-${m.role}`}>
            {m.role === 'assistant' && (
              <span className="msg-avatar" aria-hidden="true" title="Petra">
                P
              </span>
            )}
            <div className="msg-bubble">
              {m.content ? (
                ...
                <span className="typing" aria-label="Asistentica piše odgovor" role="status">
                  <span></span>
                  <span></span>
                  <span></span>
                </span>
        ...
        {messages.length === 0 && !busy && (
          <div className="chat-empty">
            <span className="chat-empty-avatar" aria-hidden="true">P</span>
            <p className="chat-empty-hint">
              Bok, ja sam <strong>Petra</strong>. Učim s Vama menadžment ljudskih potencijala —
              odaberite pitanje ili upišite svoje.
            </p>
        ...
        <textarea
          ...
          placeholder={
            voiceMode
              ...
              : listening
                ? 'Prepisujem govor…'
                : 'Postavite pitanje iz ljudskih potencijala…'
          }
          maxLength={2000}
          disabled={busy}
      ...
      <p className="chat-disclaimer">
        Odgovore generira umjetna inteligencija na temelju knjige <em>Menadžment ljudskih
        potencijala</em> i mogu sadržavati pogreške. Servis je edukativan i ne zamjenjuje stručni,
        pravni ni kadrovski savjet. Molimo ne unosite osobne podatke.
      </p>
        ...
        >
          ...
          >
            <div className="sources-modal-head">
              <strong>Izvori — poglavlja knjige</strong>
              <button
                type="button"
            ...
            <ul>
              {messages[sourcesIdx]!.sources!.map((s) => (
                <li key={s.url}>
                  {/* Odjeljci knjige (knjiga://…) nisu web poveznice — prikazuju se
                      kao tekst; prave poveznice (http…) otvaraju se u pregledniku. */}
            ...
            <p className="sources-modal-note">Menadžment ljudskih potencijala</p>
          </div>
        </div>
      )}
