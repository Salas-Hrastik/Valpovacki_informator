/**
 * Dijagnostika dohvata (RAG) — pokazuje GDJE neki dokument pada u pretraživanju.
 *
 *   npm run rag:debug -- "Kada se održava Ljeto valpovačko?"
 *   npm run rag:debug -- "Kada se održava Ljeto valpovačko?" --find=58-ljv-cover
 *
 * Ispisuje top rezultate vektorskog i leksičkog (FTS) kanala s rezultatom
 * (score) i rangom, te POSEBNO gdje se nalazi traženi dokument (--find, zadano
 * "58-ljv-cover"). Tako vidimo je li ispod praga sličnosti, izvan skupa
 * kandidata ili ga reranker izbaci. Treba SUPABASE_URL + SERVICE_ROLE_KEY i
 * embedding ključ (kao ingest) — iz okoline ili .env.local.
 */
...
async function main(): Promise<void> {
  ...
  const find = (args.find((a) => a.startsWith('--find=')) || '--find=58-ljv-cover').split('=')[1];
  if (!query) {
    console.error('Uporaba: npm run rag:debug -- "<pitanje>" [--find=<dio-URL-a>]');
    process.exit(2);
  }
  ...
  console.log(`[rag-debug] Tražim dokument koji sadrži: "${find}"`);
